...
impl AppAPI {
    ...
    pub fn handle_message(&mut self) -> Uint8Array {
        let decoded: MsgToEngine = bincode::deserialize(&self.msg_data[..]).unwrap();

        let output_ev = self.app.update(decoded);

        let encoded: Vec<u8> = bincode::serialize(&output_ev).unwrap();
